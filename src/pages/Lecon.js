...
const Lecon = () => {
  ...
  const handleSubmit = (data) => {
    //setShowModal(true)
    request
      .post(endPoint.lecons, data, header)
      .then((res) => {
  ...
  const handleEditSubmit = (data) => {
    //setShowModal(true)
    request
      .post(endPoint.lecons + "/" + editId, data, header)
      .then((res) => {
        console.log("Enregistrer avec succès");
        setEditId("");
        setRefresh(refresh + 1);
  ...
  const onDelete = () => {
    request
      .delete(endPoint.lecons + "/" + viewData.slug, header)
      .then((res) => {
        console.log(res.data);
        setRefresh(refresh + 1);
      })
              ...
              <div>
                ...
                <span className="d-inline-block">
                  {viewData.chapitre?.label +
                    " : " +
                    viewData.chapitre?.matiere_de_la_classe?.matiere?.abreviation +
                    "/" +
