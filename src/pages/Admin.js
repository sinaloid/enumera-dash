...
const Admin = () => {
  ...
  const getAll = () => {
    request
      .get(endPoint.administrateurs, header)
      .then((res) => {
        setDatas(res.data.administrateurs);
        /// getAllCategorie();
        console.log(res.data.administrateurs);
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  const handleSubmit = (data) => {
    //setShowModal(true)
    request
      .post(endPoint.livreurs, data, header)
      .then((res) => {
  ...
  const handleEditSubmit = (data) => {
    //setShowModal(true)
    request
      .put(endPoint.livreurs + "/" + editId, data, header)
      .then((res) => {
        console.log("Enregistrer avec succès");
        setEditId("");
        setRefresh(refresh + 1);
  ...
  const onDelete = (id) => {
    request
      .delete(endPoint.livreurs + "/" + id, header)
      .then((res) => {
        console.log(res.data);
        setRefresh(refresh + 1);
      })
